...
/**
 * PORTABLE TEXT COMPONENTS (Logika Arab + Jeda Paragraf)
 */
const ptComponents = {
  block: {
    normal: ({ children }: any) => {
      ...
      return <p>{children}</p>; // CSS margin diatur di PostClient global
    },
  },
};
    ...
    <div style={{ background: "#fff", minHeight: "100vh" }}>
      ...
      <div className="container" style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 20px" }}>
        ...
        <nav style={{ marginBottom: 35, fontSize: 13, color: "#888" }}>
          <Link href="/" style={{ fontWeight: "bold", color: "#5D427C", textDecoration: "none" }}>Home</Link> /
          <span style={{ color: "#5D427C" }}> {post.category || currentCat}</span> /
        ...
        <div className="main-layout-grid">
          ...
          <main style={{ minWidth: 0 }}>
            <h1 style={{ fontSize: "clamp(28px, 5vw, 42px)", fontWeight: 900, marginBottom: 35, lineHeight: 1.2 }}>
              {post.title}
            </h1>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f0eaf5", paddingBottom: 25, marginBottom: 45, flexWrap: "wrap", gap: 20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 15 }}>
                <img src="/hermawati.png" className="author-avatar" alt="Hermawati" />
                <div style={{ display: "flex", flexDirection: "column" }}>
                  <span style={{ fontWeight: 800, color: "#5D427C" }}>Hermawati</span>
              ...
              {/* SHARE BUTTONS (LENGKAP 4) */}
              <div style={{ display: "flex", gap: 10 }}>
                <a href={`https://www.facebook.com/sharer/sharer.php?u=${fullUrl}`} target="_blank" style={shareCircle("#1877F2")}>
                ...
                <a href={`https://api.whatsapp.com/send?text=${fullUrl}`} target="_blank" style={shareCircle("#25D366")}>
                ...
                <a href={`https://t.me/share/url?url=${fullUrl}`} target="_blank" style={shareCircle("#0088cc")}>
                ...
                <a href={`https://pinterest.com/pin/create/button/?url=${fullUrl}&media=${post.image}`} target="_blank" style={shareCircle("#E60023")}>
            ...
            {post.image && <img src={post.image} className="featured-image" alt={post.title} />}
            ...
            {/* RELATED POSTS (Locked Grid) */}
            {relatedPosts.length > 0 && (
              <div style={{ marginTop: 60, paddingTop: 40, borderTop: "2px solid #f0eaf5" }}>
                <h3 style={{ marginBottom: 25, color: "#5D427C" }}>Artikel Terkait</h3>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: 25 }}>
                  {relatedPosts.map((rel: any) => (
                    <Link key={rel._id} href={`/category/${rel.categorySlug || "artikel"}/${rel.slug}`} style={{ textDecoration: "none", color: "inherit" }}>
                      <div className="related-img-box"><img src={rel.image} alt={rel.title} /></div>
                      <h4 style={{ fontSize: 15, fontWeight: 700, margin: 0, lineHeight: 1.4 }}>{rel.title}</h4>
                    </Link>
                  ))}
                </div>
              </div>
          ...
          <aside>
            <div className="sidebar-sticky-wrapper">
              <h3 style={{ fontSize: 18, marginBottom: 25, borderLeft: "4px solid #B294D1", paddingLeft: 12 }}>Paling Populer</h3>
              {popularPosts.map((p: any, i: number) => (
                <Link key={p._id} href={`/category/${p.categorySlug || "artikel"}/${p.slug}`} className="pop-item">
                  <span style={{ fontSize: 18, fontWeight: 900, color: "#B294D1", opacity: 0.3 }}>{i + 1}</span>
                  <div className="pop-img-box"><img src={p.image} alt={p.title} /></div>
                  <div className="pop-info">
                    <h5 style={{ margin: 0, fontSize: 14, fontWeight: 700, color: "#333", lineHeight: 1.4 }}>{p.title}</h5>
                  </div>
                </Link>
              ))}
            </div>
